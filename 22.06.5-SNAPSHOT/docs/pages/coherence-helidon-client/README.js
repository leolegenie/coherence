...
<h2 id="_coherence_java_cdi_grpc_client">Coherence Java CDI gRPC Client</h2>
<div class="section">
...
<div class="section">
<p>The Coherence gRPC Helidon client is a CDI enabled library that allows Java clients to connect via gRPC to a
Coherence proxy server. This library has a dependency on <a id="" title="" target="_blank" href="https://helidon.io/#/">Helidon</a> for some services.</p>

<p>In order to use Coherence gRPC Helidon client, you need to declare it as a dependency in your <code>pom.xml</code></p>
...
>    &lt;dependency&gt;
        ...
        &lt;version&gt;22.06.5-SNAPSHOT&lt;/version&gt;
    &lt;/dependency&gt;</markup>

<div class="admonition note">
<p class="admonition-inline">Using the Coherence gRPC Helidon client assumes that there is a corresponding server process running which is
using the Coherence gRPC proxy service to expose the required gRPC endpoints.</p>
</div>
...
<router-link to="#coherence-cdi/README.adoc" @click.native="this.scrollFix('#coherence-cdi/README.adoc')">Coherence CDI</router-link> documentation.</p>
...
>grpc:
  channels:
    default:                   <span class="conum" data-value="1" />
...
<h3 id="sessions">Coherence gRPC Sessions</h3>
<div class="section">
...
<h4 id="_session_types">Session Types</h4>
<div class="section">
<p>By default, all sessions configured in Helidon configuration are gRPC client sessions.
The type can be specifically set using the <code>type</code> property for the session configuration.
There are two valid values for the <code>grpc</code> and <code>coherence</code>.</p>

<ul class="ulist">
<li>
<p>A <code>grpc</code> session type specified that the session is a gRPC client session.</p>

</li>
<li>
<p>A <code>coherence</code> type specifies that the session wraps a <code>ConfigurableCacheFactory</code> loaded from a Coherence
...
>coherence:
  sessions:
    products:                          <span class="conum" data-value="1" />
      type: grpc
      serializer: pof
      channel: prod
    extend:                            <span class="conum" data-value="2" />
      type: coherence
      configUri: coherence-config.xml</markup>

<ul class="colist">
<li data-value="1">The <code>products</code> session has a <code>type: grpc</code> property so it will be a gRPC client session.</li>
<li data-value="2">The <code>`extend</code> session will wrap a <code>ConfigurableCacheFactory</code> using the <code>coherence-config.xml</code> config file.</li>
</ul>
</div>
</div>
...
>@Inject
@SesionName("products")  <span class="conum" data-value="1" />
...
When using them with the gRPC Java client.</p>
...
>@Inject
@Name("products")   <span class="conum" data-value="1" />
private Session session;</markup>

<ul class="colist">
<li data-value="1">The <code>@Name</code> qualifier has the value <code>products</code>, so the <code>Session</code> injected here will be the pre-configured
<code>Session</code> named <code>products</code>.</li>
</ul>
</div>
...
to maps or caches from a specific <code>Session</code>. This is achieved by specifying a value for the <code>@SessionName</code> qualifier.
...
<p>In the example above the <code>@SesionName</code> qualifier has a value <code>test</code>, so the events will only be observed from the <code>people</code>
